...
async def _handle_publish_command(session_id: str) -> list[dict[str, Any]]:
    ...
    lines = [f"**🔬 protoResearcher{instance_tag} Weekly Digest — {date_str}**\n"]
    ...
    lines.append(f"\n_Generated by protoResearcher{instance_tag} — protoLabs.studio_")

    digest_content = "\n".join(lines)

    # Publish via webhook
    import httpx
    webhook_name = f"protoResearcher [{instance_name}]" if instance_name else "protoResearcher"
    payload = {
